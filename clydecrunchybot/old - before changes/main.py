...
from fetch_disney_proxies import (
    ...
    refresh_disney_proxies_periodically,
    _disney_proxy_pool,
    get_pool_size,
...
def create_steam_games_file(result: dict, user_plan: str, timestamp: str) -> str | None:
    ...
    total_games = result.get('games_count', len(games))
    ...
    lines.append(f"  ⏳ Total Play : {total_playtime:,} hours")
    if twofa:
    ...
    lines.append("-" * 60)
    lines.append(f"  {'#':<5} {'Game Name':<40} {'Playtime':>8}")
    lines.append(f"  {'-'*5} {'-'*40} {'-'*8}")
    ...
    for i, game in enumerate(games, start=1):
        ...
        if len(name) > 38:
            name = name[:35] + "..."
        lines.append(f"  {i:<5} {name:<40} {playtime:>6}h")

    lines.append("-" * 60)
    lines.append(f"  Total: {len(games)} games listed | {total_playtime:,} hours total")
    lines.append("=" * 60)
    lines.append("")
...
def create_steam_games_file_bulk(hits: list, user_plan: str, timestamp: str) -> str | None:
    ...
    # Combined totals
    combined_games = sum(h.get('games_count', 0) for h in hits_with_games)
    ...
    for idx, hit in enumerate(hits_with_games, start=1):
        ...
        total_games = hit.get('games_count', len(hit['games']))
        ...
        lines.append("-" * 60)
        lines.append(f"  Total: {len(games)} games | {total_playtime:,} hours")
        lines.append("=" * 60)
    ...
    for idx, hit in enumerate(hits_with_games, start=1):
        email = hit.get('email', 'N/A')
        g = hit.get('games_count', 0)
        p = hit.get('total_playtime', 0)
        lines.append(f"  ✅ Hit #{idx:<3} {email:<35} {g:>4} games | {p:,}h")
    lines.append("-" * 60)
...
def get_proxyless_banner(proxy_enabled: bool = False, pool_size: int = 0) -> str:
    if proxy_enabled and pool_size > 0:
        return f"""🌐 <b>Proxy Mode</b> ✅
- Proxy rotation active ({pool_size} proxies)
...
def get_checker_function(api_mode: str, user_id: int = None):
    ...
    if user_id:
        if user_id == ADMIN_ID:
            ...
            if is_global_proxy_enabled():
                proxy_url = get_next_disney_proxy()
        else:
            ...
            if plan in ["BASIC", "VIP", "YEARLY", "OWNER"] and user_proxy_enabled:
                proxy_url = get_next_disney_proxy()
    ...
    checkers = {
        ...
        "Steam": lambda email, password: check_steam(email, password),
        "ExpressVPN": lambda email, password: check_expressvpn(email, password),
        "Disney+": lambda email, password: check_disneyplus(email, password, proxy_url=proxy_url, user_id=user_id),
...
async def lifespan(app: FastAPI):
    ...
    # Fetch Disney+ proxies at startup, refresh every 30 min
    threading.Thread(target=refresh_disney_proxies_periodically, daemon=True).start()
...
Status: <code>{'ON ✅' if (stats.get("proxy_enabled", False) if user_id != ADMIN_ID else is_global_proxy_enabled()) else 'OFF ❌'} ({get_pool_size()} live proxies)</code>
━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━
{get_proxyless_banner(stats.get("proxy_enabled", False) if user_id != ADMIN_ID else is_global_proxy_enabled(), get_pool_size())}
...
🌍 <b>Country:</b> {country_display if country_code not in ["Unknown", "ZZ", "", "UNKNOWN"] else "Not Set by User"}"""
        ...
        if user_plan in ["VIP", "YEARLY", "OWNER"]:
            ...
            # Ban info
            text += f"\n━━━━━━━━━━━━━━━━━━━━━━━"
            vac_status = "⚠️ VAC BANNED" if result.get('vac_banned') else "✅ Clean"
        ...
        if user_plan in ["YEARLY", "OWNER"]:
            ...
            if result.get('vac_banned'):
                text += f"\n⚠️ <b>VAC Bans:</b> <code>{result.get('number_of_vac_bans', 0)}</code>"
                text += f"\n📅 <b>Days Since Ban:</b> <code>{result.get('days_since_last_ban', 0)}</code>"

            # Recent games (last 2 weeks)
            if result.get('recent_games'):
                text += f"\n🕹️ <b>Recent Games (2 weeks):</b>"
                for game in result['recent_games'][:5]:
                    text += f"\n   • {game['name']} ({game['playtime_2weeks']}h)"
...
def check_disneyplus(email: str, password: str, proxy_url=None, user_id: int = None):
    try:
        ...
        if looks_like_combo:
            ...
            try:
                ...
                if mode == "Steam" and result.get('success') and result.get('games') and user_plan in ["VIP", "YEARLY", "OWNER"]:
                    ...
                    if games_file:
                        total_games = result.get('games_count', len(result['games']))
                        total_playtime = result.get('total_playtime', 0)
                        ...
                        await update.message.reply_document(
                            ...
                            caption=(
                                ...
                                f"⏳ Total Playtime: <b>{total_playtime:,}h</b>\n"
                                f"──────────────────────\n"
                                f"☰ BY @caydigitals ✅\n"
                                f"──────────────────────\n"
...
async def handle_document(update: Update, context: CallbackContext):
    ...
    if document.file_name.endswith('.txt') and context.user_data.get('waiting_for_proxy_file'):
        ...
        count = set_uploaded_proxies(proxies)
        type_summary = get_proxy_type_summary()
    ...
    def check_account(acc):
        ...
        # Wait while paused (instant, no network)
        pause_start = time.time()
        while get_scan_status(scan_id) == "paused":
            status_now = get_scan_status(scan_id)
            if status_now == "stopped":
                return None
            if time.time() - pause_start > 600:
                set_scan_status(scan_id, "stopped")
                return None
            time.sleep(0.1)  # 100ms is fine, threads aren't the issue
        ...
        # Wait while paused after rate limiter
        pause_start = time.time()
        while get_scan_status(scan_id) == "paused":
            status_now = get_scan_status(scan_id)
            if status_now == "stopped":
        ...
        # Wait while paused after result
        pause_start = time.time()
        while get_scan_status(scan_id) == "paused":
            status_now = get_scan_status(scan_id)
            if status_now == "stopped":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=user_threads) as executor:
        ...
        for coro in asyncio.as_completed(futures):
            ...
            # ← ADD: only track real results, not None (stopped/skipped)
            ...
            # Notify owner about new Custom Plans (even if it's cancelled)
            ...
            if completed % 5 == 0 or completed == total:
                # ← ADD: Skip UI update entirely while paused — prevents ghost messages
                current_status = get_scan_status(scan_id)
                if current_status == "paused":
                    pass  # Don't touch the message while paused
                else:
...
async def show_proxy_manager(query, context):
    ...
    import fetch_disney_proxies

    user_id = query.from_user.id
    stats = get_user_stats(user_id)
    plan = stats.get("plan", "FREE").upper()

    if plan == "FREE" and user_id != ADMIN_ID:
        await query.answer("🔒 Proxy Manager is only available for BASIC+ plans!", show_alert=True)
        return

    pool_size = get_pool_size()
    last_refreshed = fetch_disney_proxies._disney_last_refreshed
    auto_refresh = is_auto_refresh_enabled()
    user_uploaded = is_user_uploaded_pool()
    type_summary = get_proxy_type_summary()

    # Admin uses global toggle, users use personal toggle
    if user_id == ADMIN_ID:
        proxy_enabled = is_global_proxy_enabled()
    ...
    if last_refreshed == 0.0:
        last_refreshed_text = "Never"
    else:
        mins_ago = int((time.time() - last_refreshed) / 60)
        last_refreshed_text = "Just now" if mins_ago == 0 else f"{mins_ago} min ago"

    pool_source = "📤 Uploaded" if user_uploaded else "🌐 Auto-fetched"
    ...
    auto_refresh_text = "🔁 Auto-Refresh: ON ✅ — Click to Disable" if auto_refresh else "🔁 Auto-Refresh: OFF ❌ — Click to Enable"
...
⏰ <b>Last Refreshed:</b> <code>{last_refreshed_text}</code>
🔁 <b>Auto-Refresh:</b> <code>{'ON ✅' if auto_refresh else 'OFF ❌'} (every 30 min)</code>
━━━━━━━━━━━━━━━━━━━━━━━━
✅ <b>Supported Modes:</b>
<code>- 🏰 Disney+</code>
<code>- 📚 Webtoon</code>
...
- Upload your own proxy list (.txt)
- Or fetch free proxies manually
...
━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(auto_refresh_text, callback_data="toggle_auto_refresh")],
        [InlineKeyboardButton("📤 Upload Proxy List", callback_data="upload_proxy_file")],
        [InlineKeyboardButton("🔃 Fetch Free Proxies Now", callback_data="refresh_disney_proxies")],
...
async def button_callback(update: Update, context: CallbackContext):
    ...
    if data == "menu_stats":
        ...
        await run_blocking(fetch_disney_proxies)
        await show_admin_panel(query, context, is_callback=True)
    ...
    elif data == "toggle_auto_refresh":
        new_val = not is_auto_refresh_enabled()
        set_auto_refresh_enabled(new_val)
        status = "ON ✅" if new_val else "OFF ❌"
        await query.answer(f"🔁 Auto-Refresh: {status}", show_alert=False)
    ...
    elif data == "refresh_disney_proxies":
        await query.answer("🔃 Refreshing proxy pool...", show_alert=False)
        
        # Show loading state in the proxy manager message
        try:
            await query.edit_message_text(
                "🔃 <b>Refreshing Proxy Pool...</b>\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━\n"
                "📡 Fetching & verifying proxies...\n"
        ...
        if new_mode == "Spotify":
            await query.answer("🎵 Spotify Mode coming soon! Stay tuned.", show_alert=True)
            return
